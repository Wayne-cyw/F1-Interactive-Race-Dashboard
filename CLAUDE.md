...
- **Nested flex/grid children need explicit `minHeight: 0`**: flex items default to `min-height: auto`, which refuses to shrink below content's natural size — a `flex: 1` child (e.g. OverviewTab's Track Map SVG, sized by its own aspect ratio) can silently inflate its ancestors' height and reintroduce page overflow even though the ancestor also has `flex: 1`. Fixed by adding `minHeight: 0` at each nested flex/grid level between the page root and the offending child. If a "fill available space" panel starts overflowing the page again, check for a missing `minHeight: 0` first.
- **Don't fight overflow with pixel-tuned padding — make tall content scroll internally instead**: OverviewTab's grid uses `gridTemplateRows: 'minmax(0, 1fr)'` (not implicit `auto`), and `Leaderboard`/the Telemetry column both have `overflowY: 'auto'` + `minHeight: 0`. This means the grid row is sized to whatever space is actually available (not to its tallest column's content), and any column whose content doesn't fit scrolls internally rather than pushing the whole page taller. This is robust to viewport size, font-metric shifts after web fonts load, etc. — manually trimming padding to fit one specific tested viewport height is fragile and reintroduces overflow the moment conditions change slightly.
- **Resize handles (`ResizeHandle.jsx`) are invisible until hovered** — no permanent decorative line; the panel's own edge is the only visual boundary, and hovering reveals a thin line as the cursor changes to `col-resize`/`row-resize`. Don't reintroduce a default-visible line color.
