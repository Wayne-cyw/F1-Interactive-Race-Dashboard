...
**Frontend endpoint usage (checked after connecting Race Center to the backend, 2026-08-01):** the Race Center (`frontend/pages/LiveRace.jsx` and everything under `frontend/pages/live-race/`) now actively calls `/api/seasons`, `/api/races/<year>`, `/api/session/<year>/<round>/R`, `/api/pitstops/<year>/<round>`, `/api/weather/<year>/<round>`, `/api/track/<year>/<round>`, `/api/positions/<year>/<round>`, `/api/track-status/<year>/<round>`, and `/api/telemetry/<year>/<round>/R/<driver>` — all via `useRaceReplay.js` and `useDriverTelemetry.js`. `/api/session-types`, `/api/standings`, `/api/teams`, and `/api/drivers` currently have no frontend consumer (Race Center always requests session type `R`, and has no standings/teams/roster views).
...
| `GET /api/track-status/<year>/<race>` | Track status timeline: green/yellow/VSC/safety car/red flag, relative to green flag |
...
- `frontend/pages/live-race/useRaceReplay.js` — season/race selection, session bundle fetching (`/session`, `/pitstops`, `/weather`, `/track`, `/positions`, `/track-status`), and real-time playback state (`elapsedSeconds`, `totalDurationSeconds`, `isPlaying`, `play`/`pause`/`seekToSeconds`, `playbackSpeed`/`setPlaybackSpeed`); a `clockEpoch` counter forces the animation-loop effect to restart on every external write to `elapsedSeconds` (race switch, seek, restart-from-end), since `isPlaying`/`totalDurationSeconds` alone don't reliably change on those events. The animation-frame loop itself runs every frame for accurate pacing, but only commits a state update — and therefore a re-render — at a throttled 30Hz (`RENDER_INTERVAL_MS`), not on every frame
...
- `frontend/pages/live-race/telemetrySlice.js` — `sliceTelemetry` derives what the UI needs at the current `elapsedSeconds` from those raw ~2Hz points: whole-race-so-far stats (top/avg speed, DRS activation count), a current-lap-so-far polyline trace (feeds the Telemetry tab's lap-trace charts), and three rolling 15-second-window traces for speed/throttle/brake (feeds the Overview tab, real-time-mapped so they scroll at a constant rate rather than compressing). `interpolateTelemetryPoint` (binary-search + linear interpolation for continuous fields, step-hold for discrete ones) smooths `current` between the 2Hz backend samples for the 30Hz render loop
...
- `frontend/pages/live-race/dnf.js` — computes per-driver DNF status from session results plus a last-completed-lap fallback, and the `elapsedSeconds` at which each DNF'd driver should be revealed (real retirement moment plus a confirmation window); `leaderboardData.js` uses it to mark rows `dnf: true`, sort them to the bottom (earliest retiree lowest), and swap gap text to "DNF", and `OverviewTab.jsx` uses it to drop retired drivers' markers from the track map — all recomputed live from `elapsedSeconds`, so scrubbing backward past a reveal point reverses it
...
- `frontend/pages/live-race/trackStatus.js` — `STATUS_META` maps FastF1's track-status codes (green/yellow/VSC/safety car/red flag) to a label+color; `deriveCurrentTrackStatus` finds the latest `/api/track-status` event at or before `elapsedSeconds`, the same pattern `raceClock.js`'s `deriveCurrentLap` uses for laps
- `frontend/utils/api.js` — shared `fetchJSON` wrapper (base URL, error handling)
- `frontend/pages/live-race/{TopBar,TabNav,OverviewTab,TimingTab,StrategyTab,TelemetryTab}.jsx` — one component per section; each tab is plain HTML/SVG with inline styles (no Canvas, no Three.js). `TopBar.jsx` is race/year selects + track status pill + weather — playback controls live in `PlaybackBar.jsx`, not here
- `frontend/pages/live-race/PlaybackBar.jsx` — persistent playback bar rendered outside the tab-switch conditional: play/pause, scrubber, speed control
